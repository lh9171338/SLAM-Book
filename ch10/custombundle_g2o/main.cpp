...
void SetSolverOptionsFromFlags(BALProblem* bal_problem, const BundleParams& params, g2o::SparseOptimizer* optimizer)
{
    ...
    if(params.linear_solver == "dense_schur" ){
        auto linearSolverDense = g2o::make_unique<g2o::LinearSolverDense<BalBlockSolver::PoseMatrixType>>();
        linearSolver = std::move(linearSolverDense);
    }
    else if(params.linear_solver == "sparse_schur"){
        auto linearSolverCholmod = g2o::make_unique<g2o::LinearSolverCholmod<BalBlockSolver::PoseMatrixType>>();
        linearSolverCholmod->setBlockOrdering(true);  // AMD ordering , only needed for sparse cholesky solver
        linearSolver = std::move(linearSolverCholmod);
    }


    auto solver_ptr = g2o::make_unique<BalBlockSolver>(std::move(linearSolver));
