...
int main(int argc, char** argv)
{
    ...
    for(int i=0; i<num_files; i++)
    {
        ...
        cout << "VO " << i+1 << " costs time: " << timer.elapsed() << endl;
        ...
        for(auto& mp : vo->m_map->m_mapPoints)
        {
            myslam::MapPoint::Ptr pt = mp.second;
            Vector2d p_p = ptrFrame->m_ptrCamera->world2pixel(pt->m_position, ptrFrame->m_T_cw);
            circle(img_show, Point2f(p_p[0], p_p[1]), 5, Scalar(0,255,0), 2);
        }
